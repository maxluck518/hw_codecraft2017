...
void topo_init(char * topo[MAX_EDGE_NUM],int line_num,MCMF * cdn,MCFS * qiwei){
    ...
    for( arc = qiwei->arcsP ; arc != qiwei->stopArcsP ; arc++ )
    {
        if(arc->flow!=0)
        {
            int tail=(arc->tail)-(qiwei->nodesP);
            int head=(arc->head)-(qiwei->nodesP);
            int flow=arc->flow;
            int cost=arc->cost;
            // cout<<"wei  tail="<<tail<<" head="<<head<<" flow="<<flow<<" cost="<<cost<<endl;
            // cout<<endl;

            if(head == cdn->s)
                cdn->AddSuperS(head);
            else
                cdn->AddEdge(tail,head,flow,cost);
        }
...
ga_struct init_key_serevr(graph_init *clean_graph){
    ...
    for(int i=0; i<clean_graph->tn-1; i++)
    {
        if(clean_graph->tDfct[i]!=0)
        {
